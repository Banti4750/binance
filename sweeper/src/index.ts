...
async function binanceSweeper() {
    try {
        for (;;) {
            // Get all users with sufficient balance
            const users = await UserModel.find({ balance: { $gt: MIN_ETH_SWEEP_THRESHOLD } })
            ...
            const estimatedGasFee = parseUnits(gasPriceData.gasPrice.toString(), "wei") * BigInt(estimatedGasLimit);

            // Process all users concurrently
            const sweepPromises = users.map(async (user) => {
                try {
                    //@ts-ignore
